...
const JoinUs = () => {
  ...
  const submitform = async () => {
    seterror(null); // જૂની એરર ક્લિયર કરવા માટે
          ...
          >
            {error && (
              <div className="space-y-2 mb-4">
                {error.map((val, i) => (
                  <p key={i} className="text-[10px] text-red-600 font-bold uppercase tracking-widest bg-red-50 p-3 rounded-xl border border-red-100 flex items-center gap-2">
                    <ShieldCheck size={12} /> {val.msg}
                  </p>
                ))}
              </div>
            )}
